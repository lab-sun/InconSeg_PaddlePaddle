...
class PD_RGB_Disparity_dataset(Dataset):

    def __init__(self, data_dir, split, input_h=288, input_w=512 ,transform=[]):
        super(PD_RGB_Disparity_dataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        return paddle.concat([paddle.to_tensor(image), paddle.to_tensor(depth).unsqueeze(0)],axis=0), paddle.to_tensor(label), name
